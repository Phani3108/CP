<script lang="ts">
	...
	// --- AI Portfolio Review ---
	let reviewMd = $state('');
	let reviewGeneratedAt = $state<string | null>(null);
	let reviewLoading = $state(false);

	const reviewHtml = $derived(
		reviewMd ? DOMPurify.sanitize(marked.parse(reviewMd, { async: false }) as string) : ''
	);
	...
	async function generateReview() {
		...
		reviewLoading = true;
		try {
			const res = await apiFetch('/api/v1/portfolio/review', { method: 'POST' });
			const j = await res.json().catch(() => ({}));
			if (!res.ok) throw new Error(j?.detail || 'Failed to generate portfolio review');
			reviewMd = j.review_markdown || '';
			reviewGeneratedAt = j.generated_at || null;
			toast.success('Portfolio review generated');
		} catch (e: any) {
			toast.error(e?.message || 'Failed to generate portfolio review');
		} finally {
			reviewLoading = false;
		}
	}
...
<div class="page-content" id="dashboard-main-content">
	...
	<!-- AI Portfolio Review -->
	<div class="pr-card panel" id="ai-portfolio-review">
		<div class="pr-header">
			<div class="pr-header-text">
				<div class="pr-title-row">
					<h3>AI Portfolio Review</h3>
					<span class="ai-badge" title="Generated by AI. Verify against source contracts.">
				...
				<p class="pr-caption">One-click review of expiries, auto-renewals, deviations and gaps — replaces the manual periodic review</p>
			</div>
			<div class="pr-actions">
				{#if reviewGeneratedAt}
					<span class="pr-timestamp">Generated {new Date(reviewGeneratedAt).toLocaleString()}</span>
				{/if}
				<button type="button" class="btn btn-primary" onclick={generateReview} disabled={reviewLoading}>
					{#if reviewLoading}
						<span class="spinner spinner-sm"></span>
						Generating...
					{:else if reviewMd}
		...
		{#if reviewMd}
			<div class="chat-markdown pr-body">{@html reviewHtml}</div>
